...
public class TypedargslistVisitor : Python3ParserBaseVisitor<Empty>
{
    ...
    public override Empty VisitTypedargslist([NotNull] Python3Parser.TypedargslistContext context)
    {
        ...
        while (i < n)
        {
            ...
            state.classState.currentFunctions.Peek().parameters.Add(newParameter);
            // The parameter is also a valid variable in the function, so we need
            // to reserve the name.
            state.classState.currentFunctions.Peek().variables[newParameter] = VarState.Types.Other;
            ...
            if (i + 2 < n && context.GetChild(i + 1).ToString() == "=")
            {
                ...
                state.classState.currentFunctions.Peek().defaultParameters[newParameter] = value;
                VarState.Types defaultParameterDeducedType = ParamTypeDeduction.Deduce(value);
                state.classState.currentFunctions.Peek().defaultParameterTypes[newParameter] = defaultParameterDeducedType;
                // Update the type of the variable
                state.classState.currentFunctions.Peek().variables[newParameter] = defaultParameterDeducedType;
